...
class Actor(nn.Module):
	def __init__(self, state_dim, action_dim, max_action, layer_size=256):
...
class TD3_BC(object):
	...
	def train(self, replay_buffer, batch_size=256, theta=1):
		self.total_it += 1
		...
		if self.total_it % self.policy_freq == 0:
			...
			q_mean = Q.mean()
			mse_loss = F.mse_loss(pi, action)

			# actor_loss = -lmbda * Q.mean() + F.mse_loss(pi, action) #TODO -> add parameter before F.mse
			actor_loss = -lmbda * q_mean + theta * mse_loss
			...
			for param, target_param in zip(self.actor.parameters(), self.actor_target.parameters()):
				target_param.data.copy_(self.tau * param.data + (1 - self.tau) * target_param.data)
			return critic_loss, actor_loss, lmbda, q_mean, mse_loss
